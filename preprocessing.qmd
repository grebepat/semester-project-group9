...
zurich <- st_bbox(c(xmin = 8.511829, ymin = 47.358479, xmax = 8.565388, ymax = 47.388355), crs = st_crs(4326))

# Assuming 'routes' is a list of spatial objects representing your routes
filtered_donner <- lapply(donner_raw, function(donner_raw) {
  # Convert the route to the same CRS as the bounding box
  donner_raw <- st_transform(donner_raw, st_crs(zurich))
  
  # Check if the route intersects with the Zurich bounding box
  if (any(st_intersects(route, zurich_bbox))) {
    return(route)
  } else {
    return(NULL)
  }
...
filtered_routes <- Filter(Negate(is.null), filtered_routes)
...
# load the generated APPkey from the website
...
alternative <- route(origin = df_odd, destination = df_even, transport_mode = "bicycle", traffic = FALSE, results = 4)

plot(alternative)

alternative_matrix <- route_matrix(df_odd, df_even, traffic = FALSE, transport_mode = "bicycle")

donner_vis <- donner_cor |> 
  filter(segment_id_cor == 2)
