...
| travel distance           | \[m\] | 549      | 2235  | 4049  | 370         | 2271  | 4099  |
| average gradient per 100m | \[%\] | -9.67    | 0.05  | 9.32  | -11.7       | 0.03  | 11.4  |
| max. gradient per 100m    | \[%\] | 0        | 3.75  | 69.35 | 0           | 3.23  | 66.7  |
| height up                 | \[m\] | 0.68     | 20.6  | 87.7  | 0.78        | 18.5  | 85.6  |
| height down               | \[m\] | -0.25    | -20.3 | -89.3 | -0.39       | -18.1 | -62.1 |
...
results <- traj_enriched_final %>%
  ...
  summarise(
    ...
    mean_slope = mean((slope_calc), na.rm = TRUE),
     mean_slope100 = mean((height_diff_100 / 100)) *100
...
str(alternative_final)
 ...
 ggplot(traj_enriched_final) +
   geom_boxplot(aes(type, mean_down_100))
    
    
    
t.test(original_final$tot_height_up, alternative_final$tot_height_up, paired = T)
...
ggplot(width_preference) +
  geom_boxplot(aes(x = width, y = perc, fill = type))
