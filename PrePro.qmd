...
tmap_save(tm1, "raeubertochter_raw.png", width = 250, height = 200, dpi = 250, units = "mm")
...
# ggplot() +
#   geom_sf(data = outline, alpha = 0) +
#   geom_sf(data = all_routes_seg, aes(color = segment_id), size = 0.6) +
#   scale_color_manual(values = my_palette(58)) +
#   coord_sf(datum = st_crs(2056)) +
#   facet_grid(~messenger, labeller = labeller(messenger = c(raeubertochter = "Räubertochter", donner = "Donner"))) +
#   labs(x = "E",
#        y = "N") +
#     theme_minimal() +
#   theme(
...
tm2 <- tm_shape(streets_raeubertochter, bbox = extent_raeubertochter) +
  tm_lines(lwd = "width", scale = 2.5, legend.lwd.show = FALSE) +
tm_shape(housing_raeubertochter, bbox = extent_raeubertochter) +
  tm_polygons(col = "grey25", border.col = "white", lwd = 0.5, alpha = .9) +
...
tmap_save(tm2, "raeubertochter_seg.png", width = 250, height = 200, dpi = 250, units = "mm")
...
```{r fig.width=500/25.4, fig.height=400/25.4}
...
# Then show start and ending points of trajectories
...
parrange <-  tmap_arrange(p2, p3, p4, p6, ncol = 2, nrow = 2)
tmap_save(parrange, "tunnel_workflow.png", width = 500, height = 400, dpi = 250, units = "mm")
...
alternative_lines <- route(origin = df_odd, destination = df_even, transport_mode = "bicycle", traffic = FALSE, results = 1 , routing_mode = "fast")
...
combined_vis <- st_intersection(combined, extent4)
...
tm7 <- tm_shape(streets1, bbox = extent4) + 
   tm_lines(lwd = "width", scale = 2.5, legend.lwd.show = FALSE) +
...
df_57_lines <- df_lines |> 
  filter(segment_id == 57)

df_57_points <- combined |> 
  filter(segment_id == 57)

tm_shape(df_57_lines) +
  tm_lines(col = "black") +
tm_shape(df_57_points) +
  tm_dots(col = "type") 
...
tm_shape(subset_data) +
 ...
 tmap_save(tm8, "problem.png", width = 100, height = 175, dpi = 250, units = "mm")
...
tm_shape(subset_data_interp) +
 ...
 tmap_save(tm9, "problem_solved.png", width = 100, height = 175, dpi = 250, units = "mm")


 parrange3 <-  tmap_arrange(tm8, tm9, ncol = 2, nrow = 1, outer.margins = c(0.0, 0.0, 0.0, 0.00))
tmap_save(parrange3, "problemsolving.png", width = 200, height = 175, dpi = 250, units = "mm")




###############3 Visualize alternatives and original
...
my_palette <- colorRampPalette(brewer.pal(11, "Spectral"))
 ...
 ggplot() +
  geom_sf(data = outline, alpha = 0) +
  geom_sf(data = combined, aes(color = segment_id), size = 0.6) +
   scale_color_manual(values = my_palette(22)) +
  coord_sf(datum = st_crs(2056)) +
  labs(x = "E",
       y = "N") +
